...
namespace KWS
{
    internal abstract class WaterPass : ScriptableRenderPass
    {
        ...
        readonly RenderTargetIdentifier _cameraDepthTextureRT = new RenderTargetIdentifier(
            Shader.PropertyToID("_CameraDepthTexture")
        );
        static readonly FieldInfo depthTextureFieldInfo = typeof(UniversalRenderer).GetField(
            "m_DepthTexture",
            BindingFlags.NonPublic | BindingFlags.Instance
        );

        static RTHandle _dummyRT;
        public static RTHandle dummyRT =>
            _dummyRT
            ?? (_dummyRT = RTHandles.Alloc(1, 1, colorFormat: GraphicsFormat.R8G8B8A8_UNorm));
        ...
        )
        {
            using (var builder = renderGraph.AddUnsafePass<PassData>(PassName, out var passData))
            {
                ...
                builder.UseTexture(
                    passData.cameraColorTarget,
                    ColorPassWriteAccess ? AccessFlags.Write : AccessFlags.Read
                );
        ...
        public virtual void ExecutePerFrame(
            ...
            CustomFixedUpdates fixedUpdates
        ) { }
