...
namespace MagicaCloth2
{
    ...
    public class ClothManager : IManager, IValid
    {
        ...
        internal bool CheckVisible(Animator ani, List<Renderer> renderers)
        {
            if (ani)
            {
                // Fix: Explicitly extract the 32-bit integer hash code to avoid the obsolete casting warning
                int id = ani.GetEntityId().GetHashCode();
        ...
        bool CheckRendererVisible(List<Renderer> renderers)
        {
            foreach (var ren in renderers)
            {
                if (ren)
                {
                    ...
                    int id = ren.GetEntityId().GetHashCode();
