        ...
        public class SerializeData : IDataValidate, ITransform
        {
            ...
            public override int GetHashCode()
            {
                ...
                foreach (var t in collisionBones)
                {
                    if (t)
                    {
                        // Call GetHashCode() directly on the EntityId struct
                        hash.Add(t.GetEntityId().GetHashCode());
                    }
                }
            ...
            public void ReplaceTransform(Dictionary<int, Transform> replaceDict)
            {
                for (int i = 0; i < collisionBones.Count; i++)
                {
                    ...
                    if (t)
                    {
                        // Resolves the 'EntityId obsolete' warning using the legacy signature
                        int legacyId = t.GetEntityId().GetHashCode();
                        if (replaceDict.ContainsKey(legacyId))
                        {
                            collisionBones[i] = replaceDict[legacyId];
                        }
                    }
                }
            }
