    ...
    public partial class ClothSerializeData : IDataValidate, IValid, ITransform
    {
        ...
        public override int GetHashCode()
        {
            ...
            foreach (var ren in sourceRenderers)
                // Fix: Call GetHashCode() directly on the EntityId struct
                hash += ren?.GetEntityId().GetHashCode() ?? NullHash;
            foreach (var t in rootBones)
            {
                ...
                while (stack.Count > 0)
                {
                    ...
                    // Fix: Call GetHashCode() directly on the EntityId struct
                    hash += t2.GetEntityId().GetHashCode();
            ...
            foreach (var map in paintMaps)
            {
                ...
                {
                    // Fix: Call GetHashCode() directly on the EntityId struct
                    hash += map.GetEntityId().GetHashCode();
        ...
        public void ReplaceTransform(Dictionary<int, Transform> replaceDict)
        {
            for (int i = 0; i < rootBones.Count; i++)
            {
                ...
                if (t)
                {
                    // Fix: Extract the 32-bit hash code to match the legacy int dictionary
                    int legacyId = t.GetEntityId().GetHashCode();
                    if (replaceDict.ContainsKey(legacyId))
                    {
                        rootBones[i] = replaceDict[legacyId];
                    }
                }
            }
