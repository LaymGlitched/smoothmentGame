...
namespace MagicaCloth2
{
    ...
    public partial class ClothProcess
    {
        ...
        int AddRenderer(Renderer ren, RenderSetupData referenceSetupData, RenderSetupData.UniqueSerializationData referenceUniqueSetupData)
        {
            ...
            // Fix: Request the structural 32-bit hash code directly to prevent implicit casting warnings
            int handle = ren.GetEntityId().GetHashCode();
            ...
            try
            {
                ...
                if (sdata2.boneAttributeDict.Count > 0)
                {
                    ...
                    foreach (var kv in sdata2.boneAttributeDict)
                    {
                        if (kv.Key)
                        {
                            // Fix: Call GetHashCode() on the EntityId struct to safely supply an integer key
                            boneAttributeDict.Add(kv.Key.GetEntityId().GetHashCode(), kv.Value);
                        }
                    }
                }
