    ...
    public partial class ClothProcess
    {
        ...
        internal bool GenerateBoneClothSelection()
        {
            ...
            foreach (var rt in cloth.SerializeData.rootBones)
            {
                if (rt)
                {
                    // Fix: Call GetHashCode() on the EntityId struct to safely supply the expected integer ID
                    int index = setupData.GetTransformIndexFromId(rt.GetEntityId().GetHashCode());
