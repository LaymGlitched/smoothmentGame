...
namespace MagicaCloth2
{
    ...
    public class DrawIconInHierarchy
    {
        ...
        {
            // Fix: Use the new EntityId event signature for Unity 6
            EditorApplication.hierarchyWindowItemByEntityIdOnGUI += DrawIcon;
        }

        // Fix: Change 'int instanceId' to 'EntityId entityId'
        static void DrawIcon(EntityId entityId, Rect rect)
        {
            rect.width = iconSize;

            // Fix: Use EntityIdToObject instead of InstanceIDToObject
            GameObject obj = UnityEditor.EditorUtility.EntityIdToObject(entityId) as GameObject;
            if (obj == null)
                return;
