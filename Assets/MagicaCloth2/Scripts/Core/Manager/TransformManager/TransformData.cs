...
namespace MagicaCloth2
{
    ...
    public partial class TransformData : IDisposable
    {
        ...
        /// </summary>
        /// <param name="t"></param>
        /// <param name="tid">0の場合はTransformからGetEntityId()を即時設定する</param>
        ...
        public int AddTransform(Transform t, int tid = 0, int pid = 0, byte flag = TransformManager.Flag_Read, bool checkDuplicate = true)
        {
            ...
            if (emptyStack.Count > 0)
            {
                ...
                if (tid == 0)
                {
                    ...
                    // Fix: Call GetHashCode() directly on the returned EntityId structs
                    idArray[index] = t.GetEntityId().GetHashCode();
                    parentIdArray[index] = t.parent?.GetEntityId().GetHashCode() ?? 0;
            ...
            else
            {
                ...
                if (tid == 0)
                {
                    // Transformからデータを取得（メインスレッドのみ）
                    // Fix: Call GetHashCode() directly on the returned EntityId structs
                    idArray.Add(t.GetEntityId().GetHashCode());
                    parentIdArray.Add(t.parent?.GetEntityId().GetHashCode() ?? 0);
        ...
        /// Unity.objectの(==)比較は様々な処理が入りGetEntityId()を利用してしまうため。
        /// </summary>
