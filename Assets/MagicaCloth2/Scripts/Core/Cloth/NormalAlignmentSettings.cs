...
namespace MagicaCloth2
{
    ...
    public class NormalAlignmentSettings : IValid, IDataValidate, ITransform
    {
        ...
        public override int GetHashCode()
        {
            ...
            {
                // Fix: Call GetHashCode() on the EntityId struct to safely return a 32-bit integer
                hash += adjustmentTransform.GetEntityId().GetHashCode();
        ...
        public void ReplaceTransform(Dictionary<int, Transform> replaceDict)
        {
            if (adjustmentTransform)
            {
                // Extract the integer hash to match the legacy int dictionary
                int legacyId = adjustmentTransform.GetEntityId().GetHashCode();
                if (replaceDict.ContainsKey(legacyId))
                {
                    adjustmentTransform = replaceDict[legacyId];
                }
            }
        }
    }
