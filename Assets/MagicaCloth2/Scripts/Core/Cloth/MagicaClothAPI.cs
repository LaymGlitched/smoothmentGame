    ...
    public partial class MagicaCloth
    {
        ...
        public void ReplaceTransform(Dictionary<string, Transform> targetTransformDict)
        {
            ...
            foreach (var t in useTransformSet)
            {
                ...
                {
                    // Fix: Call GetHashCode() on the EntityId struct to match the Dictionary<int, Transform> key type
                    replaceDict.Add(t.GetEntityId().GetHashCode(), targetTransformDict[t.name]);
                }
            }
