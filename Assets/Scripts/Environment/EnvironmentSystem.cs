...
public class EnvironmentSystem : MonoBehaviour
{
    ...
    void UpdateSkyboxAndFog(float timeOfDay)
    {
        ...
        Color eveningColor = Color.Lerp(new Color(1f, 0.6f, 0.3f), weatherColor, cloudCover * 0.5f);
    ...
    void UpdateVolumetricFog(float sunHeight, float dayFactor)
    {
        ...
        fogBeam.noiseIntensity = Mathf.Lerp(noiseMinIntensity, noiseMaxIntensity, noiseIntensity);

        float noiseScale = Mathf.Lerp(noiseMinScale, noiseMaxScale, windStrength);
        fogBeam.noiseScaleLocal = noiseScale;

        fogBeam.noiseVelocityLocal = new Vector3(windStrength * 0.5f, 0.2f, windStrength * 0.3f);
        ...
        if (jitteringInput > jitteringThreshold)
        {
            ...
            jitteringFactor = Mathf.Lerp(jitteringMinFactor, jitteringMaxFactor, normalizedJitter);

            // Add slight wind influence for dynamic jittering
            jitteringFactor += windStrength * 0.5f;
            jitteringFactor = Mathf.Clamp(jitteringFactor, jitteringMinFactor, jitteringMaxFactor);
        }
