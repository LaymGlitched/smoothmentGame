...
namespace Pumkin.AvatarTools
{
    ...
    public static class PrefManager
    {
        ...
        {
            // Convert EntityId to string to preserve its future structure
            string idString = obj?.GetEntityId().ToString() ?? string.Empty;
            SetString($"{prefString}.{name}", idString);
        }
        ...
        {
            string idString = GetString($"{prefString}.{name}", string.Empty);
            if (string.IsNullOrEmpty(idString)) return null;

            // Use your API's preferred parsing method (e.g., EntityId.Parse or a constructor)
            // If EntityId remains a struct, it likely supports equality comparisons directly
            return Resources.FindObjectsOfTypeAll<T>()
                .FirstOrDefault(o => o.GetEntityId().ToString() == idString);
        }
