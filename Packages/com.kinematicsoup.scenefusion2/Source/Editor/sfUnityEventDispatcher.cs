...
namespace KS.SceneFusion2.Unity.Editor
{
    ...
    public class sfUnityEventDispatcher
    {
        ...
        private void InvokeCreateEvent(ref ObjectChangeEventStream stream, int index)
        {
            ...
            int instanceId = data.entityId.ToInt();
            UObject uobj = EditorUtility.EntityIdToObject(data.entityId);
#else
            int instanceId = data.instanceId;
            UObject uobj = EditorUtility.InstanceIDToObject(data.instanceId);
#endif
            SetEventFlag(instanceId, Events.CREATE);
            if (OnCreate == null)
            {
                return;
            }
        ...
        private void InvokeCreatePrefabChildEvent(ref ObjectChangeEventStream stream, int index)
        {
            ...
            UObject uobj = EditorUtility.InstanceIDToObject(data.instanceId);
#endif
            if (!SetEventFlag(instanceId, Events.CREATE_CHILD) || OnCreatePrefabChild == null)
            {
                return;
            }
            GameObject gameObj = uobj as GameObject;
        ...
        private void InvokeDeleteEvent(ref ObjectChangeEventStream stream, int index)
        {
            ...
            int instanceId = data.instanceId;
#endif
            SetEventFlag(instanceId, Events.DELETE);
            if (OnDelete != null)
            {
                OnDelete(instanceId);
            }
        ...
        private void InvokeHierarchyStructureChangeEvent(ref ObjectChangeEventStream stream, int index)
        {
            ...
            UObject uobj = EditorUtility.InstanceIDToObject(data.instanceId);
#endif
            if (!SetEventFlag(instanceId, Events.CHANGE_HIERARCHY_STRUCTURE) ||
        ...
        private void InvokeUpdatePrefabInstanceEvents(ref ObjectChangeEventStream stream, int index)
        {
            ...
            {
                int id = entityId.ToInt();
                if (SetEventFlag(id, Events.CHANGE_HIERARCHY_STRUCTURE) && OnUpdatePrefabInstance != null)
                {
                    GameObject gameObj = EditorUtility.EntityIdToObject(entityId) as GameObject;
                    if (gameObj != null)
                    {
        ...
        private void InvokeParentChangeEvent(ref ObjectChangeEventStream stream, int index)
        {
...
#if UNITY_6000_0_OR_NEWER
            int instanceId = data.entityId.ToInt();
            int previousParentInstanceId = data.previousParentEntityId.ToInt();
            int newParentInstanceId = data.newParentEntityId.ToInt();
            if (instanceId == 0 && previousParentInstanceId == newParentInstanceId)
            {
                if (!SetEventFlag(newParentInstanceId, Events.REORDER_CHILDREN) || OnReorderChildren == null)
                {
                    return;
                }
                gameObj = EditorUtility.EntityIdToObject(data.newParentEntityId) as GameObject;
                if (gameObj != null)
                {
            ...
            if (!SetEventFlag(instanceId, Events.CHANGE_PARENT) || OnParentChange == null)
            {
                return;
            }
            gameObj = EditorUtility.EntityIdToObject(data.entityId) as GameObject;
            if (gameObj != null)
            {
        ...
        private void InvokeReorderChildrenEvent(ref ObjectChangeEventStream stream, int index)
        {
            ...
            UObject uobj = EditorUtility.InstanceIDToObject(data.instanceId);
#endif
            if (!SetEventFlag(instanceId, Events.REORDER_CHILDREN) || OnReorderChildren == null)
            {
                return;
            }
            GameObject gameObj = uobj as GameObject;
            ...
            bool invokeAddRemoveComponentsForPrefabVariant)
        {
            ...
            UObject uobj = EditorUtility.InstanceIDToObject(data.instanceId);
#endif
            bool canInvoke = SetEventFlag(instanceId, Events.CHANGE_PROPERTIES);
            ...
            if (uobj != null)
            {
                ...
                if (invokeAddRemoveComponentsForPrefabVariant)
                {
                    ...
                    if (gameObj != null && PrefabUtility.IsPartOfPrefabAsset(gameObj) &&
                        ...
                        SetEventFlag(instanceId, Events.ADD_REMOVE_COMPONENT) && OnAddOrRemoveComponents != null)
                    {
        ...
        private void InvokePropertiesChangedEventForAsset(ref ObjectChangeEventStream stream, int index)
        {
            ...
            UObject uobj = EditorUtility.InstanceIDToObject(data.instanceId);
#endif
            if (!SetEventFlag(instanceId, Events.CHANGE_PROPERTIES) || OnPropertiesChanged == null ||
        ...
        private void InvokeAddOrRemoveComponentsEvent(ref ObjectChangeEventStream stream, int index)
        {
            ...
            UObject uobj = EditorUtility.InstanceIDToObject(data.instanceId);
#endif
            if (!SetEventFlag(instanceId, Events.ADD_REMOVE_COMPONENT) || OnAddOrRemoveComponents == null)
            {
                return;
            }
